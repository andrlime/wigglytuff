...
class RabbitQueue:
    ...
    def __init__(self, queue_name: str) -> None:
        try:
            ...
            logger.info("Attempting to connect to rabbitmq://%s:%s", hostname, port)
            self.connection = pika.BlockingConnection(
                pika.ConnectionParameters(hostname, port=port, credentials=credentials)
            )
        except Exception as e:
            raise RabbitMQError("Unable to connect to RabbitMQ. Did you run `docker compose up`?") from e
    ...
    def start_consuming(self):
        self.channel.basic_consume(queue=self.queue_name, on_message_callback=self.receive_message, auto_ack=False)
